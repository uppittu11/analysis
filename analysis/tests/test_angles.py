...
class TestAngles(ConfTest):
    ...
    def sample_vectors(self):
        vec1 = [[0, 0, 1], [-1, 0, 0], [1 / np.sqrt(3), 1 / np.sqrt(3), 1 / np.sqrt(3)]]
        vec2 = [[0, 0, 1], [1, 0, 0], [0, 1 / np.sqrt(2), 1 / np.sqrt(2)]]
        vec1 = np.array(vec1)
        vec2 = np.array(vec2)
    ...
    def test_calc_direction_vector(self, sample_coords):
        ...
        expected = [[-1, 1, 0], [0, 2, 0], [0, 1, 0], [3, 1, -5]]
    ...
    def test_calc_angle(self, sample_vectors):
        ...
        expected = [0.0, 0.0, 35.2643896827547]
        assert np.allclose(result, expected)
