...
def analyze_my_shit(frame, masses):
    ...
    if bilayer:
        ...
        directors = np.array(directors)
        # Calculate Tilt Angles
        tilt.put( (pos, np.mean(calc_tilt_angle(directors))) )
        # Calculate Nematic Order Parameter
        s2.put( (pos, calc_order_parameter(directors)) )
        apl.put( (pos, frame.unitcell_lengths[0, 0]**2 / len(residues) * n_leaflets) )

def main():
    trajfile = sys.argv[1]
    try:
        topfile  = sys.argv[2]
        systeminfo = sys.argv[3]
        outputdir = sys.argv[4]
    except:
        topfile = sys.argv[1]
        systeminfo = sys.argv[2]
    ...
    # Save all my shit
    np.savetxt('{}/apl.txt'.format(outputdir), apl, header='top\tbottom', fmt='%.4f', delimiter='\t')
    np.savetxt('{}/tilt.txt'.format(outputdir), tilt, header='top\tbottom', fmt='%.4f', delimiter='\t')
    np.savetxt('{}/s2.txt'.format(outputdir), s2, header='top\tbottom', fmt='%.4f', delimiter='\t')
    np.savetxt('{}/height.txt'.format(outputdir), height, header='top\tbottom', fmt='%.4f', delimiter='\t')
    ...
    with open('{}/summary.txt'.format(outputdir), 'w') as f:
        ...
        for i in [0, 1]:
            f.write(layer[i])
            f.write('Area per Lipid:\t{0:.3f} +/- {1:.3f}\n'.format(np.mean(apl[:,i]), np.std(apl[:,i])))
            f.write('Tilt Angle:\t\t{0:.3f} +/- {1:.3f}\n'.format(np.mean(tilt[:,i]), np.std(tilt[:,i])))
            f.write('S2:\t\t\t\t{0:.3f} +/- {1:.3f}\n'.format(np.mean(s2[:,i]), np.std(s2[:,i])))
            f.write("\n")
