...
int main() {
    ...
    std::ofstream o_apt("data/apt.txt");
    o_apt << "# frame apt_top(A^2) apt_bottom(A^2)" << std::endl; 
    ...
    const char *init_components[] = {"ucer2-tails", "ucer2-headgroups",
        "ucer2-unequal-tail-part", "ucer2-fa-tail-full", "ecer2-tails", 
        "ecer2-headgroups",  
        "ffac16-tail", "ffac16-headgroups", "ffac16", "ffac24-tail", 
        "ffac24-headgroups", "ffac24", "ffac24-tail-unequal", "ffac24-tail-equal", 
        "ucer2", "ecer2", "all", "chol", 
        "chol-chmes", "chol-rings", "chol-tail", "chol-headgroup"}; 
    ...
    for(int frame = 0; frame < traj.n_frames(); frame++) {
        ...
        std::map<std::string, int> tails_per_layer; 
        lipids_per_layer["top"] = 0; lipids_per_layer["bottom"] = 0; 
        tails_per_layer["top"] = 0; tails_per_layer["bottom"] = 0; 
        for(auto &molecule : molecules) {
            ...
            if(molecule.smolecule_id == "ecer2-mapping2") {
                tails_by_layer[layer].push_back(
                        make_sub_gbb_from_group(molecule, "fa_tail")); 
                tails_by_layer[layer].push_back(
                        make_sub_gbb_from_group(molecule, "sph_tail")); 
                lipids_per_layer[layer]++; 
                tails_per_layer[layer]++;
                tails_per_layer[layer]++;
                add_groups_to_zhist(molecule, 
                        layer_hists[layer]["ecer2-headgroups"],
                        "headgroup", lipids_com[normdir], normdir);
                add_groups_to_zhist(molecule, 
                        layer_hists[layer]["ecer2-tails"],
                        "all_tail_beads", lipids_com[normdir], normdir); 
                add_groups_to_zhist(molecule,
                        layer_hists[layer]["ecer2"], "all", lipids_com[normdir], normdir);
                add_groups_to_zhist(molecule, 
                        layer_hists[layer]["all"], "all", lipids_com[normdir], normdir); 
                double tails_angle = calc_angle(molecule.v_coord[2], molecule.v_coord[5],
                        molecule.v_coord[11]);
            }
            if(molecule.smolecule_id == "chol") {
                ...
                add_groups_to_zhist(molecule, layer_hists[layer]["all"],
                        "all", lipids_com[normdir], normdir); 
                add_groups_to_zhist(molecule, layer_hists[layer]["chol-chmes"],
                        "chmes", lipids_com[normdir], normdir); 
                add_groups_to_zhist(molecule, layer_hists[layer]["chol-rings"],
                        "rings", lipids_com[normdir], normdir); 
                add_groups_to_zhist(molecule, layer_hists[layer]["chol-tail"],
                        "tail", lipids_com[normdir], normdir); 
                add_groups_to_zhist(molecule, layer_hists[layer]["chol-headgroup"],
                        "headgroup", lipids_com[normdir], normdir); 
                lipids_per_layer[layer]++;
                tails_per_layer[layer]++;
            }
            if(molecule.smolecule_id == "c16ffa") {
                tails_by_layer[layer].push_back(
                        make_sub_gbb_from_group(molecule, "tail"));
                add_groups_to_zhist(molecule, layer_hists[layer]["ffac16-headgroups"],
                        "headgroup", lipids_com[normdir], normdir);
                add_groups_to_zhist(molecule, layer_hists[layer]["ffac16"],
                        "all", lipids_com[normdir], normdir);
                add_groups_to_zhist(molecule, layer_hists[layer]["ffac16-tail"],
                        "tail", lipids_com[normdir], normdir); 
                add_groups_to_zhist(molecule, layer_hists[layer]["all"],
                        "all", lipids_com[normdir], normdir); 
                lipids_per_layer[layer]++;
        ...
        o_apt << frame << " " << area_A2/tails_per_layer["top"] << " "
            << area_A2/tails_per_layer["bottom"] << std::endl;
