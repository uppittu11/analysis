...
def calc_peaks(frame, atomselection, n_layers, masses, window=40):
    atoms = frame.top.select(atomselection) # which atoms to plot
    box_length = np.mean([frame.unitcell_lengths[0,2]])
    hist, edges = np.histogram(frame.xyz[0, atoms, 2].reshape(-1), 
                                weights=masses.take(atoms),
...
def calc_height(frame, atomselection, n_layers, masses,  window=40):
    peaks = calc_peaks(frame, atomselection, window, n_layers, masses)
    peaks = np.sort(peaks)
